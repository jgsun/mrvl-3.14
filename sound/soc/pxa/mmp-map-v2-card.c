/*
 * sound/soc/pxa/mmp-map-v20-card.c
 *
...
static int map2_startup_FM(struct snd_pcm_substream *substream)
{
...
static int map2_startup_hifi(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static int map2_startup_lofi(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static int map2_fe_hw_params(struct snd_pcm_substream *substream,
			      struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
	...
	/* For i2s2/5, the dai format is pcm */
	if ((codec_dai->id == 2) || (codec_dai->id == 5)) {
...
static int map2_be_dei2s_hw_params(struct snd_pcm_substream *substream,
			      struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static int map2_be_hw_params(struct snd_pcm_substream *substream,
		struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static int map2_aux_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
					struct snd_pcm_hw_params *params)
{
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
...
static int map2_tdm_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
					struct snd_pcm_hw_params *params)
{
	struct snd_interval *rate = hw_param_interval(params,
						SNDRV_PCM_HW_PARAM_RATE);
...
static int map2_tdm_spkr_hw_params(struct snd_pcm_substream *substream,
			      struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
void map2_tdm_spkr_shutdown(struct snd_pcm_substream *substream)
{
#ifdef CONFIG_SND_TDM_STATIC_ALLOC
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static int map2_tdm_hs_hw_params(struct snd_pcm_substream *substream,
			      struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
void map2_tdm_hs_shutdown(struct snd_pcm_substream *substream)
{
#ifdef CONFIG_SND_TDM_STATIC_ALLOC
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static int map2_tdm_mic_hw_params(struct snd_pcm_substream *substream,
			      struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
void map2_tdm_mic_shutdown(struct snd_pcm_substream *substream)
{
#ifdef CONFIG_SND_TDM_STATIC_ALLOC
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static struct snd_soc_ops map_ops[] = {
	{
	 .startup = map2_startup_hifi,
	 .hw_params = map2_hw_params,
	},
	{
	 .startup = map2_startup_hifi,
	 .hw_params = map2_fe_hw_params,
	},
	{
	 .startup = map2_startup_lofi,
	 .hw_params = map2_fe_hw_params,
	},
	{
	 .startup = map2_startup_lofi,
	 .hw_params = map2_be_dei2s_hw_params,
	},
	{
	 .startup = map2_startup_hifi,
	 .hw_params = map2_tdm_spkr_hw_params,
	 .shutdown = map2_tdm_spkr_shutdown,
	},
	{
	 .startup = map2_startup_hifi,
	 .hw_params = map2_tdm_mic_hw_params,
	 .shutdown = map2_tdm_mic_shutdown,
	},
	{
	 .startup = map2_startup_hifi,
	 .hw_params = map2_tdm_hs_hw_params,
	 .shutdown = map2_tdm_hs_shutdown,
	},
	{
	 .startup = map2_startup_FM,
	 .hw_params = map2_fe_hw_params,
	},
	{
	.startup = map2_startup_FM,
	.hw_params = map2_be_hw_params,
	},
};
...
static int map2_late_probe(struct snd_soc_card *card)
{
	int ret;
...
static struct snd_soc_card snd_soc_map2 = {
	.name = "map-v2 asoc",
	.dai_link = &map_dai[0],
	.late_probe = map2_late_probe,
};

static int map2_probe_dt(struct platform_device *pdev)
{
	...
	ret = snd_soc_of_parse_audio_routing(&snd_soc_map2, "map,dapm-route");
	if (ret) {
		...
		if (ret == 0) {
			...
			if (fixup_index == 0)
				map_dai[i].be_hw_params_fixup =
				    &map2_aux_be_hw_params_fixup;
			else if (fixup_index == 1)
				map_dai[i].be_hw_params_fixup =
				    &map2_tdm_be_hw_params_fixup;
		}

		i++;
	}

	snd_soc_map2.num_links = i;

	return 0;
}

static struct of_device_id mmp_map2_dt_ids[] = {
	{.compatible = "marvell,map-v2-card",},
	{}
};
MODULE_DEVICE_TABLE(of, mmp_map2_dt_ids);

static int map2_audio_probe(struct platform_device *pdev)
{
	int ret;
	...
	ret = map2_probe_dt(pdev);
	if (ret < 0) {
		dev_err(&pdev->dev, "map2_probe_dt failed: %d\n", ret);
		return ret;
	}
...
static int map2_audio_remove(struct platform_device *pdev)
{
...
static struct platform_driver map2_audio_driver = {
	.driver		= {
		.name	= "marvell-map-audio-v2",
		.owner	= THIS_MODULE,
		.of_match_table = mmp_map2_dt_ids,
	},
	.probe		= map2_audio_probe,
	.remove		= map2_audio_remove,
};

module_platform_driver(map2_audio_driver);

/* Module information */
MODULE_DESCRIPTION("ALSA SoC Audio MAP 2.0");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:audio-map-2.0");
