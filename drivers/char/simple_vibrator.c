...
static void vibrator_ldo_control(struct simple_vibrator_info *info, bool onoff)
{
	int ret;
...
static int get_vibrator_platdata(struct device_node *np,
					struct simple_vibrator_info *info)
{
	u32 value;
	int ret;
	ret = of_property_read_u32(np, "min_timeout", &value);
	if (ret < 0)
		pr_err("%s:vibrator min time out is not set\n", __func__);
	else
		info->min_timeout = (int)value;
	return 0;
}

int simple_control_vibrator(struct simple_vibrator_info *info,
				unsigned char value)
{
...
static void vibrator_off_worker(struct work_struct *work)
{
	...
	info = container_of(work, struct simple_vibrator_info,
				vibrator_off_work);
	simple_control_vibrator(info, VIBRA_OFF_VALUE);
}

static void on_vibrate_timer_expired(unsigned long x)
{
	struct simple_vibrator_info *info;
	info = (struct simple_vibrator_info *)x;
...
{
	struct simple_vibrator_info *info;
	info = container_of(sdev, struct simple_vibrator_info,
				vibrator_timed_dev);
	...
	if (timeout <= 0) {
		...
		simple_control_vibrator(info, VIBRA_ON_VALUE);
...
static int vibrator_probe(struct platform_device *pdev)
{
	...
	info = kzalloc(sizeof(struct simple_vibrator_info), GFP_KERNEL);
	if (!info)
		return -ENOMEM;
