...
#if GTP_GESTURE_WAKEUP
static ssize_t gestureswitch_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "GT9157 Gesture feature is %s\n", (gestureswitch ? "enable" : "disable"));
}

static ssize_t gestureswitch_set(struct device *dev,
		struct device_attribute *attr,const char *buf, size_t count)
{
	int ret;

	ret = kstrtoint(buf, 10, &gestureswitch);
	if (ret)
		return ret;
...
static DEVICE_ATTR(gestureswitch, 0644, gestureswitch_show, gestureswitch_set);
#endif
...
*/
			    switch(doze_buf[2]){
				    ...
				    case 'v':
					    ...
					    break;
				    default:
					    GTP_DEBUG("---It is not gesture,do nothing.---\n");
			    }
...
//#endif
    ...
    if(key_value || pre_key)
    {
		...
		if(!led_flag && (value > 0))
		{
			GTP_GPIO_OUTPUT(gtp_keyled_gpio, 1);
			led_flag = 1;
		}
		if(led_flag && (value == 0))
		{
			GTP_GPIO_OUTPUT(gtp_keyled_gpio, 0);
			led_flag = 0;
		}
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if (edge_wakeup_gpio >= 0) {
		ret = request_mfp_edge_wakeup(edge_wakeup_gpio,
				...
				NULL, &ts->client->dev);
		if (ret) {
			GTP_ERROR("Failed to request edge wakeup.\n");
		}
	}
#endif
    ...
    ret = GTP_GPIO_REQUEST(gtp_keyled_gpio, "GTP KEYLED PORT");
    if (ret < 0)
    {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_keyled_gpio,ret);
        ret = -ENODEV;
    }
...
static void gtp_parse_dt(struct device *dev)
{
	...
	gtp_int_gpio = of_get_named_gpio(np, "irq-gpios", 0);
	gtp_rst_gpio = of_get_named_gpio(np, "reset-gpios", 0);
	gtp_keyled_gpio = of_get_named_gpio(np, "keyled_gpios", 0);
#if GTP_GESTURE_WAKEUP
	edge_wakeup_gpio = gtp_int_gpio;
#endif
}
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	char cfg_name[18];
	if(panel_flag)
		snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group1");
	else
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (!vdd_ana) {
		vdd_ana = regulator_get(&client->dev, "avdd"); //"vdd_ana"
		if (IS_ERR(vdd_ana)) {
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ts->use_irq)
    {
        gtp_irq_enable(ts);
			...
			ret = device_create_file(&client->dev, &dev_attr_gestureswitch);
		if (ret < 0)
			dev_err(&client->dev,
				"%s: failed to add getstureswitch sysfs files: %d\n", __func__, ret);
#endif
    }
...
#if GTP_GESTURE_WAKEUP
    device_remove_file(&client->dev, &dev_attr_gestureswitch);
       if (edge_wakeup_gpio >= 0) {
               ret = remove_mfp_edge_wakeup(edge_wakeup_gpio);
               if (ret) {
                       GTP_ERROR("Failed to remove edge wakeup.\n");
               }
       }
#endif
...
#elif defined(CONFIG_PM_RUNTIME)
...
static const struct dev_pm_ops goodix_ts_pmops = {
	SET_RUNTIME_PM_OPS(gtp_pm_suspend, gtp_pm_resume, NULL)
};
